...
function resolveNodeInput(
  ...
  if (!binding || !binding.enabled || binding.source.kind !== "node_output") return null;
...
export function buildEditingInputs(
  ...
  const videos = content.manifest.ordered_video_binding_ids.flatMap((bindingId) => {
    ...
    if (binding?.input_role !== "video_reference") return [];
  ...
  const bgm = bgmBinding?.input_role === "audio_reference"
