...
native_remove_stale_pid_file "$NATIVE_AGENT_PID_FILE"
...
if native_pid_is_running "$NATIVE_AGENT_PID_FILE"; then
  printf '[AdCraft] Agent: running (PID %s)\n' "$(<"$NATIVE_AGENT_PID_FILE")"
else
  printf '[AdCraft] Agent: stopped\n'
fi
...
if curl --fail --silent --max-time 3 -H "Authorization: Bearer $NATIVE_AGENT_RUNTIME_TOKEN" "$(native_agent_health_url)" >/dev/null 2>&1; then
  printf '[AdCraft] Agent health: healthy\n'
else
  printf '[AdCraft] Agent health: unavailable\n'
fi
