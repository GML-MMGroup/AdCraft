...
Double-clicking a Script opens an inline Script workbench. Draft and Failed nodes edit `generation_prompt`, expose the existing model selector, and then run or retry through the existing node runtime callbacks. This keeps `structured_content` empty so the backend does not prematurely transition the node to Ready before execution. Ready nodes edit `structured_content.content` in place while preserving all other structured fields. The editor controls and save-request shape consume the same live runtime status as the card, so state transitions remain safe even before canonical workflow state catches up; Working nodes remain read-only. No new API or backend behavior is introduced.
