...
- [x] **Step 1: Write failing tests for a deferred SSE snapshot, complete multi-drag cleanup, full snapshot restoration, and non-finite coordinates.**

- [x] **Step 2: Run `npm test -- --run src/features/agent-canvas/canvas/draggingNodeState.test.ts` and confirm the new assertions fail because the coordination functions do not exist.**

- [x] **Step 3: Implement pure helpers that never mutate node arrays, retain the latest complete snapshot, clear the full active ID set, overlay finite final coordinates, preserve selection, and remove `dragging`.**

- [x] **Step 4: Re-run the focused test and confirm all drag-state assertions pass.**
...
- [x] **Step 1: Add failing source-integration assertions for pending snapshot refs, deferred `setNodes`, complete drag cleanup, and finite persistence.**

- [x] **Step 2: Run `npm test -- --run src/features/agent-canvas/AgentCanvasPage.chrome.test.ts` and confirm the new assertions fail against the current handlers.**

- [x] **Step 3: Add latest/pending snapshot refs, defer the synchronization effect during active drag, and rebuild nodes synchronously in `onNodeDragStop` before calling `updateNodePositions`.**

- [x] **Step 4: Run both focused test files and confirm they pass.**
...
- [x] **Step 1: Run the focused drag, layout, and Agent Canvas chrome tests.**

- [x] **Step 2: Run `npm run typecheck` and ESLint on changed frontend files.**

- [x] **Step 3: Run `npm run build` and `git diff --check`.**

- [x] **Step 4: Review the final diff for backend changes, unrelated refactors, and missing test coverage.**
