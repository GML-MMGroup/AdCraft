...
Frontend baseline: `AdCraft/main@a7987b3`

Backend authority: `/data/wenwu.meng/adWorkflow` canonical `main@f302cfac`
...
| Direct Product input authority | `POST /api/v2/workflows/{workflow_id}/guided/product-inputs`; typed `GuidedProductInputRequest/ResponseV2` | Workflow `If-Match` plus stable `Idempotency-Key` | Product validation/revision errors and `guided_product_*` | No client method | Record the endpoint, but do not use it as a second guided state machine. The canonical UI path remains Guided Interaction Submit | ALREADY SUPPORTED |
...
- Product upload, Product Guided Interaction, immutable AssetVersion references, source-only guards, typed waits, event replay, Character 0/1/N projections, Storyboard single-grid presentation, and Editing behavior: 22 focused Vitest files, 271 tests passed.
...
- Production packaging: `npm run build` passed with 813 modules transformed.
- Backend integrity: `/data/wenwu.meng/adWorkflow` remains on `main@f302cfac` with no tracked changes from this work.
