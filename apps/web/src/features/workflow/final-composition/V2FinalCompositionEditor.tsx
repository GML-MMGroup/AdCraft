    ...
    <section className="v2-composition-preview" aria-label="Composition preview">
      <div className="v2-composition-preview-stage" style={{ aspectRatio: timeline.aspect_ratio.replace(":", " / ") }}>
        {activeClips.map((clip) => {
          ...
          return clipSource.media_type === "video" ? <video key={clip.clip_id} ref={(element) => { if (element) videos.current.set(clip.clip_id, element); else videos.current.delete(clip.clip_id); }} src={mediaUrl(clipSource.public_url)} muted playsInline preload="metadata" style={mediaStyle} onTimeUpdate={(event) => { if (playing && clip.clip_id === activeClip?.clip_id) onPlayheadChange(Math.min(duration, clip.start_time + event.currentTarget.currentTime - (clip.trim_in ?? 0))); }} /> : <img key={clip.clip_id} src={mediaUrl(clipSource.public_url)} alt={clipSource.display_name} style={mediaStyle} />;
        })}
...
function TimelineTrack({ track, clips, selectedClipId, playheadSeconds, duration, onSelectClip, onSplitClip, onUpdateTrack }: { track: V2FinalCompositionTimeline["tracks"][number]; clips: V2FinalTimelineClip[]; selectedClipId: string | null; playheadSeconds: number; duration: number; onSelectClip: (id: string) => void; onSplitClip: (id: string, at: number) => void; onUpdateTrack: (trackId: string, update: { enabled?: boolean; muted?: boolean; locked?: boolean; name?: string }) => void }) {
  const { setNodeRef, isOver } = useDroppable({ id: track.track_id, data: { trackType: track.track_type, locked: track.locked } });
  return <div className={`v2-composition-timeline-track ${isOver ? "is-over" : ""} ${track.enabled ? "" : "is-disabled"}`}><span className="v2-composition-track-label"><input aria-label="Track name" value={track.name || track.track_type} maxLength={80} onChange={(event) => onUpdateTrack(track.track_id, { name: event.target.value })} /><span><button type="button" title={track.enabled ? "Disable track" : "Enable track"} aria-label={track.enabled ? "Disable track" : "Enable track"} onClick={() => onUpdateTrack(track.track_id, { enabled: !track.enabled })}>{track.enabled ? "◉" : "○"}</button><button type="button" title={track.muted ? "Unmute track" : "Mute track"} aria-label={track.muted ? "Unmute track" : "Mute track"} onClick={() => onUpdateTrack(track.track_id, { muted: !track.muted })}>{track.muted ? "M" : "♪"}</button><button type="button" title={track.locked ? "Unlock track" : "Lock track"} aria-label={track.locked ? "Unlock track" : "Lock track"} onClick={() => onUpdateTrack(track.track_id, { locked: !track.locked })}>{track.locked ? "▣" : "□"}</button></span></span><div ref={setNodeRef} className="v2-composition-track-lane" style={{ width: `${duration * PIXELS_PER_SECOND}px` }}><i className="v2-composition-playhead" style={{ left: `${playheadSeconds * PIXELS_PER_SECOND}px` }} />{clips.map((clip) => <TimelineClip key={clip.clip_id} clip={clip} disabled={track.locked} selected={clip.clip_id === selectedClipId} onSelect={onSelectClip} onSplit={onSplitClip} />)}</div></div>;
}
...
function AudioFields({ clip, onSetAudio }: { clip: V2FinalTimelineClip; onSetAudio: (clipId: string, update: Record<string, number | boolean>) => void }) { const audio = clip.audio ?? { volume: 1, muted: false, fade_in: 0, fade_out: 0 }; return <fieldset className="v2-composition-fieldset"><legend>Audio</legend><RangeField label="Volume" value={audio.volume} min={0} max={4} step={0.05} onChange={(value) => onSetAudio(clip.clip_id, { volume: value })} /><label className="v2-composition-checkbox"><input type="checkbox" checked={audio.muted} onChange={(event) => onSetAudio(clip.clip_id, { muted: event.target.checked })} /> Mute</label><NumericField label="Fade in" value={audio.fade_in} min={0} onChange={(value) => onSetAudio(clip.clip_id, { fade_in: value })} /><NumericField label="Fade out" value={audio.fade_out} min={0} onChange={(value) => onSetAudio(clip.clip_id, { fade_out: value })} /></fieldset>; }
function SubtitleStyleFields({ clip, onUpdate }: { clip: V2FinalTimelineClip; onUpdate: (clipId: string, updater: (clip: V2FinalTimelineClip) => V2FinalTimelineClip) => void }) { const style = clip.style ?? { font_size: 42, color: "#FFFFFF", position: "bottom_center" as const }; const update = (patch: Partial<typeof style>) => onUpdate(clip.clip_id, (current) => ({ ...current, style: { ...style, ...patch } })); return <fieldset className="v2-composition-fieldset"><legend>Subtitle style</legend><NumericField label="Font size" value={style.font_size} min={12} onChange={(value) => update({ font_size: Math.min(96, value) })} /><label className="v2-composition-color-field"><span>Color</span><input type="color" value={style.color} onChange={(event) => update({ color: event.target.value })} /></label><select value={style.position} onChange={(event) => update({ position: event.target.value === "top_center" || event.target.value === "center" ? event.target.value : "bottom_center" })}><option value="bottom_center">Bottom</option><option value="center">Center</option><option value="top_center">Top</option></select></fieldset>; }
function TransformFields({ clip, onUpdate }: { clip: V2FinalTimelineClip; onUpdate: (clipId: string, updater: (clip: V2FinalTimelineClip) => V2FinalTimelineClip) => void }) { const transform = clip.transform ?? { x: 0, y: 0, scale_x: 1, scale_y: 1, rotation: 0, opacity: 1, fit: "contain" as const }; const update = (patch: Partial<typeof transform>) => onUpdate(clip.clip_id, (current) => ({ ...current, transform: { ...transform, ...patch } })); return <fieldset className="v2-composition-fieldset"><legend>Transform</legend><RangeField label="Scale" value={transform.scale_x} min={0.1} max={4} step={0.05} onChange={(value) => update({ scale_x: value, scale_y: value })} /><RangeField label="X" value={transform.x} min={-1} max={1} step={0.05} onChange={(value) => update({ x: value })} /><RangeField label="Y" value={transform.y} min={-1} max={1} step={0.05} onChange={(value) => update({ y: value })} /><RangeField label="Rotation" value={transform.rotation} min={-360} max={360} step={1} onChange={(value) => update({ rotation: value })} /><RangeField label="Opacity" value={transform.opacity} min={0} max={1} step={0.05} onChange={(value) => update({ opacity: value })} /><select value={transform.fit} onChange={(event) => update({ fit: event.target.value === "cover" ? "cover" : "contain" })}><option value="contain">Contain</option><option value="cover">Cover</option></select></fieldset>; }
function SourceCard({ source, onAdd }: { source: V2FinalTimelineSource; onAdd: () => void }) { return <button className="v2-composition-source" type="button" onClick={onAdd}><span className="v2-composition-source-thumb">{source.thumbnail_url || source.public_url ? <img src={mediaUrl(source.thumbnail_url ?? source.public_url)} alt="" /> : source.media_type === "audio" ? <AssetsIcon /> : source.media_type === "image" ? <ImageIcon /> : <VideoIcon />}</span><span><strong>{source.display_name}</strong><em>{source.media_type}{source.duration_seconds ? ` · ${formatTime(source.duration_seconds)}` : ""}</em></span><PlusIcon /></button>; }
