...
Double-clicking a Script opens an inline Script workbench. Draft and Failed nodes edit `generation_prompt`, expose the existing model selector, and then run or retry through the existing node runtime callbacks. This keeps `structured_content` empty so the backend does not prematurely transition the node to Ready before execution. Ready nodes edit `structured_content.content` in place while preserving all other structured fields. Working nodes remain read-only until execution completes. No new API or backend behavior is introduced.
