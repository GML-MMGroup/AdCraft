...
The shared card renderer displays Script text using this precedence: `structured_content.content`, `structured_content.script_text`, `structured_content.text`, then `generation_prompt` or `summary_prompt`. An empty Script uses the existing text-family placeholder artwork. Script retains the standard Draft, Working, Ready, and Failed states.

Double-clicking a Script opens an inline Script workbench. The editor patches `structured_content.content`, preserves all other structured fields, exposes the existing model selector, and runs or retries through the existing node runtime callbacks. No new API or backend behavior is introduced.
...
- The Script workbench edits, saves, and runs Script content.
