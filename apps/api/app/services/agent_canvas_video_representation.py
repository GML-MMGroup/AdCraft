...
@dataclass(frozen=True, slots=True)
class VideoRepresentationSelectionV2:
    mode: VideoRepresentationModeV2
    source: str
    source_id: str
    policy_version: str
...
def select_video_representation_mode(
    *,
...
) -> VideoRepresentationSelectionV2:
    """Select the typed mode without admitting a provider Run."""
    ...
    payload = {
        ...
        "identity_safety_digest": None,
    }
    digest = sha256(
        json.dumps(payload, ensure_ascii=True, separators=(",", ":"), sort_keys=True).encode()
    ).hexdigest()
    return VideoRepresentationSelectionV2(
        mode=mode,
        source=source,
        source_id=source_id,
        policy_version=_POLICY_VERSION,
...
def resolve_video_representation_mode(
    *,
    ...
    selection = select_video_representation_mode(
