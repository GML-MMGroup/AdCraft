...
当用户现有项目副本已经包含更新后的代码时，再次运行同一个原生启动器。它会同步锁定的 Python 与 Node 依赖，停止上次由启动器管理的三个进程，再从更新后的文件启动 Agent Runtime、API 和 Web。保留 `.env` 与 `runtime-data/`；启动器也会保留本地数据库、生成媒体和已保存凭据。已经运行的进程不会自动加载全部新代码，因此每次重新部署后都要完成第 4 节验证。
